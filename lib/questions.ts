...
export interface QuestionOption {
  label: string;
  weights: Record<ConstitutionKey, number>;
}

// 단일 값 또는 배열로 매칭되는 프로필 술어
export interface ProfilePredicate {
  gender?: UserProfile['gender'] | UserProfile['gender'][];
  ageGroup?: UserProfile['ageGroup'] | UserProfile['ageGroup'][];
  married?: boolean;
  hasChildren?: boolean;
}

function matchesProfile(p: ProfilePredicate, profile: UserProfile): boolean {
...
// 동일 질문이 프로필에 따라 다른 prompt(또는 options)로 표시되도록
...
// ⚠️ 아래 질문과 가중치는 Claude 초안입니다.
// 성정론 원전에 따라 가중치를 반드시 검수해 주세요.
// 특히 각 답변이 어느 체질의 '性' 또는 '情' 패턴을 드러내는지 확인이 필요합니다.
...
export function filterQuestions(profile: UserProfile): Question[] {
  return questions
    .filter((q) => !q.showIf || matchesProfile(q.showIf, profile))
    .map((q) => {
      if (!q.promptVariants) return q;
      for (const v of q.promptVariants) {
        if (matchesProfile(v.when, profile)) {
          return { ...q, prompt: v.prompt, options: v.options ?? q.options };
        }
      }
      return q;
    });

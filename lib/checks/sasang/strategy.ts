import { EngineQuestion, EngineResult, ScoringStrategy } from '@/lib/engine/types';
...
export interface SasangAxisScores {
...
export function sasangAxisStrategy(): ScoringStrategy<ConstitutionKey, UserProfile> {
  return {
    kind: 'axis-decomposed-max',
    compute(answers, _profile, questions): EngineResult<ConstitutionKey> {
      const axis = computeAxisScores(answers, questions);
      const primary = KEY_ORDER.reduce((a, b) =>
        axis.total[a] >= axis.total[b] ? a : b,
      );
      return {
        primary,
        scores: axis.total,
...
export function getSasangPercentages(
  scores: Record<ConstitutionKey, number>,
...
export function axisNarrative(
  ...
  if (natureType === emotionType) {
    return `평소(性)와 자극받았을 때(情)가 모두 ${natureName} 경향입니다. 성정이 일치하는 타입으로, 평소엔 ${NATURE_PARTICIPLE[natureType]} 자연스러움이 있고 자극을 받으면 ${EMOTION_PARTICIPLE[natureType]} 패턴이 같은 결로 이어집니다. ${natureName}의 평생 과제 "${emotionLifeTask}"가 더욱 선명하게 다가옵니다.`;
  }
